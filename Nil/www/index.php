...
if (preg_match('#^users/delete/([0-9]+)$#', $uri, $match)) {
    (new UserAdmin())->delete((int)$match[1]);
    exit;
}

if (preg_match('#^users/role/([0-9]+)$#', $uri, $match)) {
    (new UserAdmin())->updateRole((int)$match[1]);
    exit;
}
